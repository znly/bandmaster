...
// Env can be used to configure an ElasticSearch (v1) session via the
// environment.
//
// It comes with sane defaults for a local development set-up.
type Env struct {
	Addr string `envconfig:"ADDR" default:"http://localhost:9201"`
}
...
// Config returns an ElasticSearch (v1) config using the values from the
// environment.
